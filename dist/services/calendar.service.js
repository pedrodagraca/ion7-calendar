...
var CalendarService = /** @class */ (function () {
    ...
    CalendarService.prototype.createSubsctractMonthsByPeriod = function (startTime, monthsNum, opt) {
        var _array = [];
        var _start = new Date(startTime);
        var _startMonth = new Date(_start.getFullYear(), _start.getMonth(), 1).getTime();
        for (var i = 0; i < monthsNum; i++) {
            var time = moment(_startMonth).subtract(i, 'M').valueOf();
            var originalCalendar = this.createOriginalCalendar(time);
    ...
    CalendarService.prototype.wrapResult = function (original, pickMode) {
        ...
        if (pickMode === config_1.pickModes.RANGE) {
            return {
                // @ts-ignore
        ...
        if (pickMode === config_1.pickModes.MULTI) {
            // @ts-ignore
            return original.map(function (e) { return _this.multiFormat(e.time); });
        }
        return original;
    };
    ...
    CalendarService.DEFAULT_DATE = new Date(1971, 0, 1);
